...
class FacultadListView(APIView):
    ...
    def put(self, request, pk):
        saved_facultad = get_object_or_404(Facultades.objects.all(), pk=pk)
        facultad = request.data.get('facultad')
        serializer = FacultadSerializer(instance=saved_facultad, data=facultad, partial=True)
        if serializer.is_valid(raise_exception=True):
            facultad_saved = serializer.save()
        return Response({"success": "Facultad '{}' updated successfully".format(facultad_saved.facultad_nombre)})
    
    def delete(self, request, pk):
        facultad = get_object_or_404(Facultades.objects.all(), pk=pk)
        facultad.delete()
