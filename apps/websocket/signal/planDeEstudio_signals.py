...
def announce_new_plan_de_estudio(sender, instance, created, **kwargs):
    if created:
        ...
        async_to_sync(channel_layer.group_send)(
            "cambios", dict(type="cambios", model="plan_de_estudio", event="c", data=model_to_dict(instance))
        )
...
def announce_update_plan_de_estudio(sender, instance, created, **kwargs):
    if not created:
        ...
        channel_layer = get_channel_layer()
        async_to_sync(channel_layer.group_send)(
            "cambios", dict(type="cambios", model="plan_de_estudio", event="u", data=model_to_dict(instance))
        )
...
def announce_del_plan_de_estudio(sender, instance, **kwargs):
    ...
    channel_layer = get_channel_layer()
    async_to_sync(channel_layer.group_send)(
        "cambios", dict(type="cambios", model="plan_de_estudio", event="d", data=model_to_dict(instance))
    )
