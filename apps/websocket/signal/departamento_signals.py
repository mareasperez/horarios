...
def announce_new_departamento(sender, instance, created, **kwargs):
    if created:
        ...
        async_to_sync(channel_layer.group_send)(
            "cambios", dict(type="cambios", model="departamento", event="c", data=model_to_dict(instance))
        )
...
def announce_update_departamento(sender, instance, created, **kwargs):
    if not created:
        ...
        channel_layer = get_channel_layer()
        async_to_sync(channel_layer.group_send)(
            "cambios", dict(type="cambios", model="departamento", event="u", data=model_to_dict(instance))
        )
...
def announce_del_departamento(sender, instance, **kwargs):
    ...
    channel_layer = get_channel_layer()
    async_to_sync(channel_layer.group_send)(
        "cambios", dict(type="cambios", model="departamento", event="d", data=model_to_dict(instance))
    )
