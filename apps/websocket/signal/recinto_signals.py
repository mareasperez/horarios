...
def announce_new_recinto(sender, instance, created, **kwargs):
    if created:
        ...
        async_to_sync(channel_layer.group_send)(
            "cambios", dict(type="cambios", model="recinto", event="c", data=model_to_dict(instance))
        )
...
def announce_update_recinto(sender, instance, created, **kwargs):
    if not created:
        ...
        channel_layer = get_channel_layer()
        async_to_sync(channel_layer.group_send)(
            "cambios", dict(type="cambios", model="recinto", event="u", data=model_to_dict(instance))
        )
...
def announce_del_recinto(sender, instance, **kwargs):
    ...
    channel_layer = get_channel_layer()
    async_to_sync(channel_layer.group_send)(
        "cambios", dict(type="cambios", model="recinto", event="d", data=model_to_dict(instance))
    )
